...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="resources/css/shareboard.css">
...
<body>
  ...
  <div id="container">
    <div id="nav_container">
      <div class="shareboard_header">
        <div class="shareboard_title">
          <form action="shareBoard" method="get" id="sbSearchForm" onsubmit="return sbSearchForm();">
        ...
        <c:if test="${not empty sessionScope.loginMember}">
          <a href="boardWrite?boardno=4"><i class="fas fa-pen-square"></i></a>
        </c:if>
      </div>
      <c:choose>
        <c:when test="${not empty boardList}">
          <table class="shareboard">
            <tr>
              <th>번호</th>
              <th>제목</th>
              <th>글쓴이</th>
      ...
      <c:if test="${not empty boardList}">
        <div class="pagination">
          ...
          <div class="bt_prev">
            <a class="prev_bt" href="shareBoard?page=${blockNum * paginationCount}<c:out value="${searchKeyword}"/>">
            ...
            <c:forEach begin="1" end="${paginationCount}" varStatus="status">
              ...
              <c:if test="${thisPage <= pageCount}">
                <a <c:out value="${currentPage == thisPage ? 'class=paging-active' : ''}"/> 
                  href="shareBoard?page=${thisPage}<c:out value="${searchKeyword}"/>">
                  <span>${thisPage}</span>
                </a>
              </c:if>
            </c:forEach>
          ...
          <div class="bt_next">
            <a class="next_bt" href="shareBoard?page=${(blockNum + 1) * paginationCount + 1}<c:out value="${searchKeyword}"/>">
      	...
      	$(".b_sicon2").click(function () {
            if ($(".board_search").val().trim() == "") {
              defocused();
        		...
        		location.href='shareBoard?keyword=' + $(".board_search").val().trim();
            }
        });
